...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const blendDiscRef = useRef<HTMLDivElement>(null);
  ...
  const constants = useMemo(() => ({ borderWidth: 18, cornerSize: 12 }), []);
  ...
  useEffect(() => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      if (blendDiscRef.current) {
        gsap.to(blendDiscRef.current, { scale: 1, duration: 0.2, ease: 'power2.out' });
      }
      ...
      const leaveHandler = () => {
        ...
        if (blendDiscRef.current) {
          gsap.to(blendDiscRef.current, { scale: 0.25, duration: 0.2, ease: 'power2.out' });
        }
    ...
    hideDefaultCursor, isMobile, hoverDuration, parallaxOn,
  ...
  return (
    ...
    >
      {/* Blend mode invert circle */}
      <div
        ref={blendDiscRef}
        className="absolute top-1/2 left-1/2 w-8 h-8 rounded-full -translate-x-1/2 -translate-y-1/2 bg-white"
        style={{ willChange: 'transform', mixBlendMode: 'difference', transform: 'translate(-50%, -50%) scale(0.25)' }}
      />
      <div
        ref={dotRef}
        className="absolute top-1/2 left-1/2 w-1.5 h-1.5 rounded-full -translate-x-1/2 -translate-y-1/2 bg-white"
